...
function startPageChangeDetection() {
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    if (lastUrl !== window.location.href || lastTitle !== document.title) {
      console.log('Navigation detected! URL or title changed:');
      console.log('Previous URL:', lastUrl);
      console.log('Current URL:', window.location.href);
      console.log('Previous title:', lastTitle);
      ...
      setTimeout(() => {
        ...
        if (CheckoutAutomation && CheckoutAutomation.isCheckoutPage()) {
          console.log('New page is a checkout page, triggering automatic analysis');
  ...
  window.addEventListener('locationchange', function() {
    ...
    setTimeout(() => {
      if (CheckoutAutomation && CheckoutAutomation.isCheckoutPage()) {
        console.log('New page after locationchange is a checkout page');
  ...
  window.addEventListener('popstate', function() {
    ...
    setTimeout(() => {
      if (CheckoutAutomation && CheckoutAutomation.isCheckoutPage()) {
        console.log('New page after popstate is a checkout page');
...
function applyFormFill() {
  ...
  function processNextAction(index) {
    if (index >= totalActions) {
      // All form actions completed, check for navigation buttons
...
function startAutomation() {
  ...
  if (!statusDiv || !statusMessage) {
    ...
    const statusDivRetry = document.getElementById('automation-status');
    const statusMessageRetry = document.getElementById('status-message');
    const applyBtnRetry = document.getElementById('apply-form-fill-btn');
    
    if (statusDivRetry && statusMessageRetry) {
      console.log('Status elements created successfully');
      // Update our references
      statusDiv = statusDivRetry;
      statusMessage = statusMessageRetry;
      applyBtn = applyBtnRetry;
    } else {
      ...
      response => {
        ...
        if (response && response.success) {
          ...
          if (applyBtn) {
            ...
            // Ensure event listener is properly set
            ...
            // Auto-apply form fill after a short delay if auto-fill is enabled
            if (window.automationAdvancedSettings && window.automationAdvancedSettings.auto_fill_forms === true) {
              console.log('Auto-fill is enabled, automatically applying form fill');
              setTimeout(() => {
                applyFormFill();
...
function hideLoadingSpinner() {
  const loadingIndicator = document.querySelector('.loading-indicator .ai-spinner');
  if (loadingIndicator) {
